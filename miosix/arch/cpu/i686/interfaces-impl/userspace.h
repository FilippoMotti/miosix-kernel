...
inline unsigned int SyscallParameters::getParameter(unsigned int index) const {
  ...
  auto *psp =
      reinterpret_cast<unsigned int *>(archPtr[STACK_OFFSET_IN_CTXSAVE]);
  return psp[index];
}

inline void SyscallParameters::setParameter(unsigned int index,
                                            unsigned int value) {
  assert(index < MAX_NUM_SYSCALL_PARAMETERS);
  auto *psp =
      reinterpret_cast<unsigned int *>(archPtr[STACK_OFFSET_IN_CTXSAVE]);
...
enum FaultType {
  ...
  MP_NOADDR =
      3,     // Process attempted data access outside its memory (missing addr)
...
inline void MPUConfiguration::IRQenable() {
  ...
  MPU->CTRL =
      MPU_CTRL_HFNMIENA_Msk | MPU_CTRL_PRIVDEFENA_Msk | MPU_CTRL_ENABLE_Msk;
#else
  // ARMv7-M
