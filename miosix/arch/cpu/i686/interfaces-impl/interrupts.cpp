...
extern char
    _main_stack_top asm("_irq_stack_top"); // defined in the linker script
 ...
 * NOTE: more compact assembly is produced if this function is not marked
 * noexcept
 */
template <int N>
void irqProxy() /*noexcept*/
{
...
template <fnptr... args> struct InterruptProxyTable {
  const fnptr entry[sizeof...(args)] = {args...};
};

template <unsigned N, fnptr... args> struct TableGenerator {
  typedef typename TableGenerator<N - 1, &irqProxy<N - 1>, args...>::type type;
};

template <fnptr... args> struct TableGenerator<0, args...> {
...
struct InterruptTable {
  constexpr InterruptTable(char *stackptr, fnptr i1, fnptr i2, fnptr i3,
                           fnptr i4, fnptr i5, fnptr i6, fnptr i7, fnptr i8,
                           fnptr i9, fnptr i10, fnptr i11, fnptr i12, fnptr i13,
                           fnptr i14, fnptr i15)
      : stackptr(stackptr), i1(i1), i2(i2), i3(i3), i4(i4), i5(i5), i6(i6),
        i7(i7), i8(i8), i9(i9), i10(i10), i11(i11), i12(i12), i13(i13),
        i14(i14), i15(i15) {}
...
/// hardware expects it
__attribute__((section(".isr_vector"))) extern const InterruptTable
...
static void tryPrintingProgramCounter() {
  ...
  const unsigned int off = 24; // Offset in bytes of PC in the stack frame on
                               // ARM
  if (psp < reinterpret_cast<unsigned int>(&_end) ||
      psp > reinterpret_cast<unsigned int>(&_heap_end) - off -
                sizeof(unsigned int))
    IRQerrorLog("? (Corrupted stack)\r\n");
  else
    printUnsignedInt(*reinterpret_cast<unsigned int *>(psp + off));
}
#endif // WITH_ERRLOG
...
#else
    SCB->SHCSR &=
        ~(SCB_SHCSR_BUSFAULTPENDED_Msk | SCB_SHCSR_MEMFAULTPENDED_Msk |
          SCB_SHCSR_USGFAULTPENDED_Msk | SCB_SHCSR_SVCALLPENDED_Msk);
#endif
    return;
  }
#else  // WITH_PROCESSES
...
#endif // WITH_PROCESSES
{
  ...
  if (cfsr & 0x00000080) // SCB_CFSR_MMARVALID
  {
    ...
    IRQerrorLog("The address that caused the fault is missing\r\n");
...
#endif // WITH_PROCESSES
{
  ...
  if (cfsr & 0x00008000) {
    ...
    id = fault::BF_NOADDR;
  if (Thread::IRQreportFault(FaultData(id, arg))) {
    // Clear BFARVALID, LSPERR, STKERR, UNSTKERR, IMPRECISERR, PRECISERR,
    // IBUSERR
    SCB->CFSR = 0x0000bf00;
    // Clear all pending exceptions as a defensive measure
    //(see comment in MEMFAULTPENDED)
...
#endif // WITH_PROCESSES
{
  ...
  if (cfsr & 0x02000000)
    ...
    id = fault::UF_UNALIGNED;
  else if (cfsr & 0x00080000)
    id = fault::UF_COPROC;
  else if (cfsr & 0x00040000)
    id = fault::UF_EXCRET;
  else if (cfsr & 0x00020000)
    id = fault::UF_EPSR;
