...
/// ((10+16+1)*4=108Bytes). Only sp, r4-r11, EXC_RETURN, s16-s31, psplim are
/// saved here, since r0-r3,r12,lr,pc,xPSR, old sp and s0-s15,fpscr are saved by
...
/// \internal Size of additional context saved on the stack during context
/// switch. If zero, this architecture does not save anything on stack during
/// context save. Size is in bytes, not words. MUST be divisible by 4. This
/// constant is used to increase the stack size by the size of context save
/// frame. (8+17)*4=100Bytes
...
/// \internal Size in words of vector to store CPU context during context switch
/// on x86 all registers are saved onto the stack, so we save only the ESP
const unsigned char CTXSAVE_SIZE = 1;

/// \internal Size of additional context saved on the stack during context
/// switch. On the stack we save EFLAGS, CS, EIP, EAX, ECX, EDX, EBX, ESP, EBP,
/// ESI, EDI at max, each one is 4 bytes (SS and ESP do not count as we are
/// still in ring 0)
const unsigned int CTXSAVE_ON_STACK = 44;
